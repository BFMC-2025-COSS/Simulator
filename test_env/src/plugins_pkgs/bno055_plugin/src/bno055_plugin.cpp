...
namespace gazebo
{
    ...
    {   
        ...
        void BNO055::Load(physics::ModelPtr model_ptr, sdf::ElementPtr sdf_ptr)
        {
            ...
            timer = nh->createTimer(ros::Duration(0.0), std::bind(&BNO055::OnUpdate, this)); // 100Hz 업데이트
            ...
            this->m_pubBNO = this->m_ros_node->advertise<sensor_msgs::Imu>(topic_name, 10);
            ...
            if(DEBUG)
            {
                ...
                ROS_INFO_STREAM("[bno055_plugin] Useful data: orientation (quaternion), angular velocity, linear acceleration");
                ROS_INFO_STREAM("====================================================================\n\n");
            }
        }
        ...
        {        
            ...
            // Set linear acceleration using WorldLinearAccel()
            imu_msg.linear_acceleration.x = this->m_model->WorldLinearAccel().X();  
            imu_msg.linear_acceleration.y = this->m_model->WorldLinearAccel().Y();  
            imu_msg.linear_acceleration.z = this->m_model->WorldLinearAccel().Z();
            ...
            imu_msg.orientation_covariance[0] = 0.005;  // Roll
            imu_msg.orientation_covariance[4] = 0.005;  // Pitch
            imu_msg.orientation_covariance[8] = 0.01;   // Yaw (더 높은 불확실성)

            imu_msg.angular_velocity_covariance[0] = 0.01;  // Roll rate
            imu_msg.angular_velocity_covariance[4] = 0.01;  // Pitch rate
            imu_msg.angular_velocity_covariance[8] = 0.02;  // Yaw rate

            imu_msg.linear_acceleration_covariance[0] = 0.01;  // X
            imu_msg.linear_acceleration_covariance[4] = 0.01;  // Y
            imu_msg.linear_acceleration_covariance[8] = 0.02;  // Z, 불확실성이 큼
